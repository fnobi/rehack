...
class Rehack extends EventEmitter {
    ...
    startWalking () {
        ...
        async.series([(next) => {
            ...
            walker.on('file', (root, stat, cb) => {
                ...
                this.convertFile(filePath, patterns, cb);
            });
    ...
    convertDir (dirPath, patterns, cb) {
        let newDirPath;
        ...
        async.series([(next) => {
            ...
            patterns.forEach((pattern) => {
                if (!newDirPath) {
                    while ((newDirPath || dirPath).indexOf(pattern[0]) >= 0) {
                        newDirPath = dirPath.replace(pattern[0], pattern[1]);
                    }
                }
            });
            if (newDirPath) {
                if (this.useGitCmd) {
                    child_process.exec(
                        util.format('git mv %s %s', dirPath, newDirPath),
                        next
                    );
                } else {
                    fs.rename(dirPath, newDirPath, next);
                }
            } else {
                setImmediate(next);
            }
    ...
    convertFile (filePath, patterns, cb) {
